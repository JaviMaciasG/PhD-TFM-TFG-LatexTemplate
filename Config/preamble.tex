...
\usepackage{listings}
% numberbychapter: ties \thelstlisting to the chapter/appendix counter, the
% same way figure and table numbers already work in this template.
...
% ---------------------------------------------------------------------------
% General-purpose code listing style, reused across the whole memoria (short
% in-chapter snippets and the full listings in the "Código" appendix).
% Supports at least Python and JavaScript (both natively known by listings)
% and JSON (via the \lstdefinelanguage{json} loaded above from
% json-lang-lstlisting.tex). Select the language per listing, e.g.:
%   \begin{lstlisting}[style=TFGcode, language=Python, caption={...}, label={lst:...}]
%   \begin{lstlisting}[style=TFGcode, language=json, caption={...}, label={lst:...}]
% ---------------------------------------------------------------------------
