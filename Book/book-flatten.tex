...
También damos las gracias a Manuel Villaverde, David Casillas, Jesús Pablo Martínez, José Francisco
Velasco Cerpa, Roberto Chamorro, Carlos Cruz, Cristina Losada.
...
\item Primero hay que preparar la versión base sobre la que luego se
  ...
  expandido. Luego hay que registrarlo en el repositorio con un
  \texttt{cvs commit -m ``New flattened version'' book-flatten.tex} o
  el estándar add y commit que se usa en git.
...
\texttt{Cbluebox} (ademÃ¡s de usar el entorno \texttt{codefloat} para
...
O incluso como en el listado \ref{cod:sample2}, usando un layout mÃ¡s refinado (con
...
Y podemos reutilizar estilos cambiando algÃºn parÃ¡metro, como podemos ver
...
Y tambiÃ©n podemos poner ejemplos de código \textit{coloreado}, como se
...
Finalmente aquÃ­ tenÃ©is un ejemplo de código shell, usando el estilo
...
\texttt{algorithm2e} para incluir algoritmos, y hay ajustes especÃ­ficos
y dependientes de este paquete tanto en \texttt{Config/preamble.tex}
como en \texttt{cover/extralistings.tex} (editadlos segÃºn vuestras
necesidades). 
...
algoritmo~\ref{alg:howto}, y otro un poco mÃ¡s complicado en el
